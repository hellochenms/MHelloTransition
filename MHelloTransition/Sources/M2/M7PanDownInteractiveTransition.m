...
static double kPanTotalAndScreenHeightFactor = 1;
...
- (instancetype)initWithType:(M7PDIActionType)type {
    ...
    if (self) {
        _type = type;
        _panTotalAndScreenHeightFactor = kPanTotalAndScreenHeightFactor;
...
- (void)onPan:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            self.panTotalHeight = CGRectGetHeight([UIScreen mainScreen].bounds) * self.panTotalAndScreenHeightFactor;
...
- (void)setPanTotalHeightScreenHeightFactor:(double)panTotalHeightScreenHeightFactor {
    _panTotalAndScreenHeightFactor = fmin(fmax(panTotalHeightScreenHeightFactor, .1), 1);
}
